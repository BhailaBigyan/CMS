...
function getSessionDataFromHeader() {
    $sessionData = null;
    $headers = apache_request_headers();
    if (isset($headers['Cookie'])) {
        $sessionCookie = explode(';', $headers['Cookie']);
        foreach ($sessionCookie as $cookie) {
            if (strpos($cookie, 'PHPSESSID') !== false) {
                $sessionData = substr($cookie, strpos($cookie, '=') + 1);
                break;
            }
        }
    }
...
$sessionData = getSessionDataFromHeader();

$connect = mysqli_connect('localhost', 'root', '', 'php_cms');

// Fetch latest blog posts
$postsQuery = "SELECT id, title, description, image, category, author, created_at FROM blog_posts ORDER BY created_at DESC LIMIT 6";
